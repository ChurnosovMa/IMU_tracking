...
// Вращение вектора v через кватернион q (q должен быть нормализован)
static BNO_Vec3_t rotateVectorByQuaternion(
    const BNO_Vec3_t v,
    float qw, float qx, float qy, float qz)
{
    // q ⊗ [0, v] ⊗ q*
    // Оптимизированная формула без полного перемножения кватернионов
...
// Получить вектор B в мировой СК
BNO_Vec3_t BNO_getMagWorldFrame(void)
{
    BNO_Vec3_t mag_body = {
    ...
    // Кватернион из ROTATION_VECTOR (уже нормализован датчиком)
    float qx = sensorData.SenVal.RotationVector.I;
    float qy = sensorData.SenVal.RotationVector.J;
    float qz = sensorData.SenVal.RotationVector.K;
    float qw = sensorData.SenVal.RotationVector.Real;

    return rotateVectorByQuaternion(mag_body, qw, qx, qy, qz);
}
